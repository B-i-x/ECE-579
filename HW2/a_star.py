...
"""In order to print our map clearly, we can use 🟦 to represent all '0's, we use 🟨 to represent all '1's, and we will use 🟩 to represent the path in the future.
...
def print_maze(maze):
    ...
    )  ## we print the maze line by line
    ...
    for i in range(len(maze_copy)):
        for j in range(len(maze_copy[0])):
            if maze_copy[i][j] == "0":
                ...
                maze_copy[i][j] = "🟨"
    maze_copy[explore_point[0]][explore_point[1]] = "🟥"
    print(
        "\n".join("".join(row) for row in maze_copy)
    )  ## we print the maze line by line
...
class_1 = {"id1": "Alice", "id2": "John", "id3": "Peter"}
...
while open_list:
    step = step + 1
    current = min(
        open_list, key=lambda x: f_score.get(x, float("inf"))
    )  # find the points with minimal f score
    open_list.remove(
        current
    )  # we remove the current point from open_list because we don't need to further consider it
    ...
    if step < 5:
        ...
        print(
            "we now explore: ",
            current,
            ", which is the one with minimal f score from the previous openlist",
        )
    ...
    for neighbor in get_neighbors(maze, current[0], current[1]):
        ...
        if neighbor not in g_score or tentative_g_score < g_score[neighbor]:
            came_from[neighbor] = (
                current  # record where this neighor came from, for retriving final path
            )
    ...
    if step < 5:
        ...
        print(
            "and they have the f scores: ",
            {key: f_score[key] for key in f_score if key in open_list},
        )
